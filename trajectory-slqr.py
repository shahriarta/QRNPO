...
def my_opt_solver(T, K0):
    ...
    plt.plot(list(range(0, T_plot + 1)),
             list(map(lambda kk: (lqr_cost(kk) - lqr_cost(K_opt))/(lqr_cost(K_r[0]) - lqr_cost(K_opt)), K_r[0: T_plot + 1])))
    plt.plot(list(range(0, T_plot + 1)),
             list(map(lambda kk: (lqr_cost(kk) - lqr_cost(K_opt))/(lqr_cost(K_euc[0]) - lqr_cost(K_opt)), K_euc[0: T_plot + 1])))
    plt.plot(list(range(0, T_plot + 1)),
             list(map(lambda kk: (lqr_cost(kk) - lqr_cost(K_opt))/(lqr_cost(K_pg[0]) - lqr_cost(K_opt)), K_pg[0: T_plot + 1])))
...
if __name__ == '__main__':
    ...
    K_0 = [insert_sparsity([-0.05,-1.3]), insert_sparsity([0.4, -1.2]), insert_sparsity([0.0, -0.5]), insert_sparsity([-0.1, 0.6]), insert_sparsity([-0.3, -0.6]), insert_sparsity([0.03, -2.2]), insert_sparsity([0.3, -1.8])]
    T = [100, 100, 200, 1400, 150, 600, 200]
    ...
    ip = InsetPosition(ax1, [0.2, 0.1, 0.3, 0.3])
    ax2.set_axes_locator(ip)
    mark_inset(ax1, ax2, loc1=1, loc2=2, fc="none", ec='black')
    ax2.set(ylim=([-1.03, -0.97]), xlim=([-0.07, 0.02]))

    for i in range(len(K_0)):
        K_r, K_euc, K_pg, K_opt, stepone = my_opt_solver(T[i], K_0[i])
        T_plot = T[i]
        ax1.plot(list(K_r[i].ravel()[indices][0] for i in range(T_plot+1)), list(K_r[i].ravel()[indices][1] for i in range(T_plot+1)), color='tab:blue')
        ax1.plot(list(K_euc[i].ravel()[indices][0] for i in range(T_plot+1)), list(K_euc[i].ravel()[indices][1] for i in range(T_plot+1)), color='tab:orange')
        ax1.plot(list(K_pg[i].ravel()[indices][0] for i in range(T_plot+1)), list(K_pg[i].ravel()[indices][1] for i in range(T_plot+1)), color='tab:green')


        ax2.plot(list(K_r[i].ravel()[indices][0] for i in range(T_plot + 1)), list(K_r[i].ravel()[indices][1] for i in range(T_plot + 1)),
                 color='tab:blue')
        ax2.plot(list(K_euc[i].ravel()[indices][0] for i in range(T_plot + 1)), list(K_euc[i].ravel()[indices][1] for i in range(T_plot + 1)),
                 color='tab:orange')
        ax2.plot(list(K_pg[i].ravel()[indices][0] for i in range(T_plot + 1)), list(K_pg[i].ravel()[indices][1] for i in range(T_plot + 1)),
                 color='tab:green')

        if not np.isnan(stepone[0]):
            ax1.plot(K_r[stepone[0]].ravel()[indices][0], K_r[stepone[0]].ravel()[indices][1], marker='s', color='tab:blue', markersize=4)
            ax2.plot(K_r[stepone[0]].ravel()[indices][0], K_r[stepone[0]].ravel()[indices][1], marker='s', color='tab:blue', markersize=4)
        if not np.isnan(stepone[1]):
            ax1.plot(K_euc[stepone[1]].ravel()[indices][0], K_euc[stepone[1]].ravel()[indices][1], marker='s', color='tab:orange', markersize=4)
            ax2.plot(K_euc[stepone[1]].ravel()[indices][0], K_euc[stepone[1]].ravel()[indices][1], marker='s', color='tab:orange', markersize=4)

    ax1.plot(K_opt.ravel()[indices][0], K_opt.ravel()[indices][1], marker='*', color='red')
    ax2.plot(K_opt.ravel()[indices][0], K_opt.ravel()[indices][1], marker='*', color='red')

    x = np.arange(-0.75, 0.5, 0.005)
    y = np.arange(-1.7, 1.7, 0.005)
    X_, Y_ = np.meshgrid(x, y)
    zs = np.array(list(map(lambda i, j: lqr_cost(K_opt + insert_sparsity([i, j])) if np.max(
        np.abs(la.eigvals(A_cl(K_opt + insert_sparsity([i, j]))))) < 1 else np.infty, np.ravel(X_), np.ravel(Y_))))
    Z_ = zs.reshape(X_.shape)

    cs = ax1.contour(K_opt.ravel()[indices][0] + X_, K_opt.ravel()[indices][1] + Y_, Z_, levels=lqr_cost(K_opt) * np.logspace(0.001, 3),
    ...
    plt.show()
    fig.savefig('images/landscape_slqr.pdf', format='pdf')
